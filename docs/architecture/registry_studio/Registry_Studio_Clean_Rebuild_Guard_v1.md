...
System prompt сохраняется на английском языке в проверенной форме Translator prototype.
Перевод prompt на другой язык или добавление новых правил считается изменением provider behavior и требует отдельного подтверждения.
...
Базовый user-content без дополнительных параметров сохраняет формат prototype:

```text
Source text:
"фактический исходный текст"
```

`SOURCE LANGUAGE HINT` и `ENGINEER CONTEXT` являются существующим контролируемым расширением Registry Studio.
Они добавляются в user-content только при фактическом наличии и не изменяют system prompt.
