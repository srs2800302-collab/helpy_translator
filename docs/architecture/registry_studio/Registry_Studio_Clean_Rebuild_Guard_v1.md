...
`assets/registry_studio/helpy/service_intake_semantic_manifest_v1.json` отдельно владеет stable internal keys и typed references для scenarios, questions, answer options, qualifiers, photo questions, photo limits и client/master guidance. Semantic manifest обязан покрывать актуальный заявленный scope соответствующей версии identity manifest и не содержать неизвестных IDs.
На текущем этапе блок Helpy service-intake состоит из 27 сущностей. Это число описывает только фактический текущий состав блока и не является архитектурным invariant, лимитом schema, manifest, adapter, Core или dependency graph.
Блок Helpy service-intake является расширяемым: допускается добавление новых root-категорий, подкатегорий, услуг, бизнес-моделей, самостоятельных сущностей и сценариев внутри каждой root-категории без изменения Core-контрактов.
Полный Registry индексируется независимо от project-specific semantic manifests. Semantic manifest является semantic overlay над уже доступными Registry entities, обогащает выбранный scope typed references и не ограничивает видимость остальных веток Registry.
