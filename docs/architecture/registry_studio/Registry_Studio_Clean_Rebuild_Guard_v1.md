...
- не останавливает последующие translation requests и audit;
...
- literal requests продолжают выполняться;
- полный девятисекционный результат передаётся независимому аудиту;
...
- зависимые последующие запросы не вызываются;
...
- продолжение literal requests и audit после таких семантических расхождений;
...
- продолжение зависимых запросов и audit после такого текста;
- запрет объяснительных переводов и добавления отсутствующего назначения во всех направлениях RU, EN и TH;
- запрет зависимых запросов после настоящего translation format failure;
