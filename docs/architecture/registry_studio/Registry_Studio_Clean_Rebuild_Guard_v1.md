...
Одна операция `translatePhrase` выполняет ровно два Typhoon request:

1. Один translation request формирует полный набор из девяти translation sections.
2. Один independent audit request проверяет полный валидированный набор из девяти sections.

Первый request возвращает строго в одном ответе:
...
Отдельные English literal request и Thai literal request не выполняются.

Reverse sections переводят фактически созданные секции `EN` и `TH`.
Они не восстанавливают предполагаемый исходный текст, не исправляются по `SOURCE TEXT` и не скрывают semantic drift.

### Утверждённый Typhoon API preset

Translation и audit requests используют проверенный на Translator prototype preset:
...
Preset является частью интеграционного контракта Registry Studio.
Изменять отдельные параметры, sampling behavior или token limits без отдельного подтверждённого engineering decision запрещено.

### Prompt полного мультиязычного перевода
...
Используй SOURCE LANGUAGE HINT только как подсказку.
Используй ENGINEER CONTEXT только для понимания терминологии или устранения неоднозначности.
Не переводи ENGINEER CONTEXT и не добавляй его сведения в переводимый текст.

Ты обязан вернуть ровно 9 секций.
...
2. В секции исходного языка повтори SOURCE TEXT без изменений.
3. Сформируй версии на русском, английском и тайском языках.
4. Переводи точно, естественно и настолько дословно, насколько позволяет грамматика целевого языка.
5. Возвращай кратчайший естественный и общепринятый эквивалент.
6. Сохраняй точный инженерный смысл и терминологию.
7. Сохраняй обязательность, допустимость, запрет и отрицание.
8. Сохраняй границы, исключения, количества, единицы измерения, условия и последовательность.
9. Не улучшай и не упрощай формулировку.
10. Не заменяй конкретный термин более общим.
11. Не добавляй пояснение назначения предмета или действия.
12. Не добавляй отсутствующий контекст.
13. EN_TO_RU и EN_TO_TH должны переводить фактическое содержимое EN.
14. TH_TO_RU и TH_TO_EN должны переводить фактическое содержимое TH.
15. Не исправляй reverse translations по SOURCE TEXT и не скрывай несовпадения.
16. Не выполняй audit.
...
EN_TO_RU:
русский перевод фактической секции EN

TH_TO_RU:
русский перевод фактической секции TH

EN_TO_TH:
тайский перевод фактической секции EN

TH_TO_EN:
английский перевод фактической секции TH
```
...
7. `SOURCE TEXT` и секция исходного языка сохраняются ровно в том виде, в котором их вернул Typhoon.
8. Их несовпадение с пользовательским исходным текстом является диагностическим evidence, а не translation format failure.
...
Текст до первой обязательной секции не является translation format failure, если все обязательные labels присутствуют, уникальны, следуют в правильном порядке и содержат валидные значения.

Такой текст:

- не включается в значение первой секции;
- сохраняется как техническая диагностика без исправления;
- не останавливает последующий audit request;
- сам по себе не изменяет semantic verdict.

При несовпадении фактического `SOURCE TEXT` с пользовательским исходным текстом либо секции исходного языка с фактическим `SOURCE TEXT`:

- фактические значения не исправляются и не подменяются;
- полный девятисекционный translation result сохраняется без подмены;
- audit request продолжает выполняться;
- итоговый статус устанавливается в `canonicalDrift`;
- `comment` содержит пользовательский исходный текст, фактический `SOURCE TEXT`, фактическую секцию исходного языка и полное пояснение аудита.

При настоящем нарушении translation format:

- audit request не вызывается;
- provider возвращает `TranslatorPhraseResult` со статусом `failed`;
- все уже строго распознанные секции сохраняются без изменений;
- фактический `SOURCE TEXT` используется, если он уже был распознан;
- пользовательский исходный текст используется только как fallback;
- причина технической ошибки передаётся инженеру через `comment`.

`Failed` означает отсутствие достоверного полного машинного вердикта, а не отсутствие уже полученных данных.
...
Все девять уже собранных translation sections сохраняются в `TranslatorPhraseResult` без исправления, дополнения или подмены. Причина невозможности сформировать достоверный verdict передаётся инженеру через `comment`.

### Запрещённое поведение

Запрещено:

- разделять полный девятисекционный translation result на отдельные direct и literal requests;
- формировать меньше девяти translation sections в первом Typhoon request;
- исправлять reverse sections по `SOURCE TEXT`;
- скрывать несовпадение прямых и обратных переводов;
- изменять утверждённый Typhoon API preset без отдельного engineering decision;
- принимать отсутствующие секции как пустые строки;
- запускать audit после неполного translation result;
...
- ровно два Typhoon request: translation и audit;
- полный девятисекционный результат в первом translation request;
- отсутствие `SOURCE LANGUAGE HINT` и `ENGINEER CONTEXT` в audit request;
- утверждённый API preset `temperature: 0.0`, `top_p: 1.0`, `frequency_penalty: 0.0`;
- token limits `700` для translation и `500` для audit;
- сохранение фактического `SOURCE TEXT` из ответа Typhoon;
- преобразование несовпадения `SOURCE TEXT` в `canonicalDrift`;
- преобразование несовпадения секции исходного языка в `canonicalDrift`;
- продолжение audit после семантических расхождений внутри полного translation result;
- отображение пользовательского и фактически возвращённого текста в диагностике;
- отказ при отсутствующей, пустой, дублированной или неожиданной секции;
- отказ при placeholder value;
- принятие полного валидного ответа с текстом до первой обязательной секции;
- сохранение такого текста в технической диагностике;
- продолжение audit после такого текста в полном translation result;
- запрет объяснительных переводов и добавления отсутствующего назначения во всех направлениях RU, EN и TH;
- запрет audit request после настоящего translation format failure;
- сохранение уже распознанных секций при позднем `failed`;
- отказ при неполном или невалидном audit output;
- сохранение всех девяти translation sections при audit failure;
